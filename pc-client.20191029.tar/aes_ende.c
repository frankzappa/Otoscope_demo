...
void AddRoundKey (int round)
{
        ...
        for (i = 0; i < 4; i++) {
                for (j = 0; j < 4; j++)
                { g_state[j][i] ^= g_RoundKey[round * Nb * 4 + i * Nb + j]; }
        }
}
                ...
                unsigned char * sKey, int nSize_skey, unsigned char * out_Encry)
{
        ...
        if (nNumRounds != 128 && nNumRounds != 192 && nNumRounds != 256)
